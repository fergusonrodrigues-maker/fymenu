...
const EmptyStateUnidade = dynamic(() => import("./components/EmptyStateUnidade"), { ssr: false, loading: () => loadingFallback });
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .dark .dash-icon-box { background: rgba(0,255,174,0.1); color: #00ffae; }
        html:not(.dark) .dash-icon-box { background: rgba(0,176,122,0.1); color: #00b07a; }
        .dark .card:hover .dash-icon-box { transform: scale(1.08); background: rgba(0,255,174,0.15); }
        html:not(.dark) .card:hover .dash-icon-box { transform: scale(1.08); background: rgba(0,176,122,0.15); }
        @keyframes iconPulse { 0%,100% { transform: scale(1); } 50% { transform: scale(1.05); } }
        .dash-icon-box-pulse { animation: iconPulse 2s ease-in-out infinite; }
        .card:hover .dash-icon-box-pulse { animation: none; transform: scale(1.08); }
        .dash-dots {
          position: fixed;
          inset: 0;
          z-index: 0;
      ...
      <div style={{
        ...
        {unit === null && allUnitsLoaded && allUnits.length === 0 && (
          <EmptyStateUnidade onCreateClick={() => open("criar-unidade")} />
        )}
        ...
        {(unit !== null || !allUnitsLoaded || allUnits.length > 0) && (() => {
          ...
          const CARD_SVGS: Record<string, React.ReactNode> = {
            analytics:   <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/><line x1="2" y1="20" x2="22" y2="20"/></svg>,
            ...
            crm:         <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
            whatsapp:    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>,
            ...
            tv:          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="15" rx="2"/><polyline points="17 2 12 7 7 2"/></svg>,
            plano:       <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
          ...
          const IconBox = ({ id, pulse }: { id: string; pulse?: boolean }) => (
            <div className={`dash-icon-box${pulse ? " dash-icon-box-pulse" : ""}`}>
              {CARD_SVGS[id] ?? CARD_CONFIGS[id]?.icon}
            </div>
          );
                    ...
                    <div key="analytics" className={cardClass} onClick={blockedClick ?? (() => open("analytics"))} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <div className="dash-icon-box" style={{ marginBottom: 0 }}>{CARD_SVGS.analytics}</div>
                        </div>
                      </div>
