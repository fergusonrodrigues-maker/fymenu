...
export default function PlanoModal({ restaurant }: { restaurant: Restaurant; trialDays: number; onUpgrade: () => void; onClose: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingTop: 8 }}>
      {/* Plano atual */}
      <div style={{ padding: "16px 20px", borderRadius: 16, background: "var(--dash-accent-soft)", marginBottom: 20 }}>
        ...
        {restaurant.status === "trial" && <span style={{ fontSize: 11, color: "var(--dash-warning)", marginTop: 2, display: "block" }}>Período de teste</span>}
      </div>
      ...
      {PLANS.map(plan => (
        <div key={plan.key} style={{
          ...
          background: plan.highlight ? "var(--dash-accent-soft)" : "var(--dash-card)",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{
                fontSize: 13,
                color: plan.highlight ? "var(--dash-accent)" : "var(--dash-text-dim)",
            ...
            {restaurant.plan === plan.key ? (
              ...
              <button
                ...
                style={{
                  ...
                  background: plan.highlight ? "var(--dash-accent-soft)" : "var(--dash-card-hover)",
          ...
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            {plan.features.map(f => (
              <div key={f} style={{ fontSize: 12, color: "var(--dash-text-muted)", display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ color: "var(--dash-accent)" }}>✓</span> {f}
              </div>
            ))}
          </div>
        </div>
