...
export default function PlanoModal({ restaurant, onClose }: { restaurant: Restaurant; trialDays: number; onUpgrade: () => void; onClose: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingTop: 8 }}>
      ...
      <div style={{ padding: "16px 20px", borderRadius: 16, background: "rgba(0,255,174,0.06)", marginBottom: 20 }}>
        ...
        {restaurant.free_access && <span style={{ fontSize: 11, color: "#00ffae" }}>Acesso gratuito</span>}
      </div>
      ...
      {PLANS.map(plan => (
        <div key={plan.key} style={{
          ...
          border: plan.highlight ? "1px solid rgba(0,255,174,0.15)" : "1px solid rgba(255,255,255,0.06)",
          position: "relative",
        }}>
          {plan.label && (
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.8px", background: "linear-gradient(135deg,#00ffae,#00d9ff)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 6 }}>
              {plan.label}
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: plan.highlight ? "rgba(0,255,174,0.8)" : "rgba(255,255,255,0.5)", fontWeight: 700, marginTop: 2 }}>
            ...
            {restaurant.plan === plan.key ? (
              ...
              <button onClick={() => window.location.href = "/painel/planos"} style={{
